...
//if(iDir == 0 && xRead != 99999) {
  fVol = xRead/100000+fVol;      // if injecting, subtract xRead from current vol
  inVol = -xRead/100000+inVol;      //initVol-fVol;
  xRead = 0;                     // clear xRead so values dont add unless new reading recorded
   }
if(boolDir == true && xRead != 99999) {
//  if(iDir == 1 && xRead != 99999) {
  fVol = -xRead/100000+fVol;      // if withdrawing, add xRead from current vol
  inVol = xRead/100000+inVol;
  xRead = 0;
   }
}
